...
| `inventory_current_v1.json` | `evidence/inventory_current.sql` | `INVENTORY_DETAILS` | ✅ **Re-extraído 2026-09-10**: Laura corrió la consulta a mano en Snowflake y `scripts/rebuild_inventory.py` armó el JSON (439 empresas · 6.801.539 ítems, +1,5% vs agosto). ⛔ Sigue **bloqueado desde el MCP** (no existe `INVENTORY_DETAILS_SV`), así que el refresco es manual. Pendiente: la segunda consulta, la de totales de red — `total_unique_products/categories/varieties` están en `null` |
...
| `hardgoods_v2.json` | `evidence/hardgoods.sql` | `SALES_SV` | ✅ **Re-extraído completo 2026-09-10** vía `scripts/rebuild_hardgoods.py`. La división sale de `product_category_division_name` (Fresh Cut / Hard Goods / **Plants**), NO de `inventory_division`, que solo tiene Boxes/Units/Hard Goods |
| `catalog_reach_v1.json` | `evidence/catalog_reach.sql` | `SALES_SV` + `PROCUREMENTS_SV` | ✅ **Nuevo 2026-09-10** vía `scripts/rebuild_catalog_reach.py`. Alcance online del catálogo (categorías/variedades/SKUs) en venta y compra. Reemplaza el gap de `skus_online_offline`, que restaba conteos solapados y daba negativo en 141 de 330 cuentas. Ventana fija de 12 meses: no sigue el selector |
